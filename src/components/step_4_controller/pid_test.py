...
class NMPCController():
    ...
    def solver_setup(self):
        ...
        # Position (x, y, theta)
        gx = self.X[0::self.n][1:] - self.X[0::self.n][:-1] - 0.5 * self.h * (
                ((self.X[3::self.n][1:] + self.X[4::self.n][1:]) / 2) * np.cos(self.X[2::self.n][1:]) + (
                (self.X[3::self.n][:-1] + self.X[4::self.n][:-1]) / 2) * np.cos(self.X[2::self.n][:-1]))
        gy = self.X[1::self.n][1:] - self.X[1::self.n][:-1] - 0.5 * self.h * (
                ((self.X[3::self.n][1:] + self.X[4::self.n][1:]) / 2) * np.sin(self.X[2::self.n][1:]) + (
                (self.X[3::self.n][:-1] + self.X[4::self.n][:-1]) / 2) * np.sin(self.X[2::self.n][:-1]))
        gtheta = self.X[2::self.n][1:] - self.X[2::self.n][:-1] - 0.5 * self.h * (
                ((self.X[3::self.n][1:] - self.X[4::self.n][1:]) / self.L) + (
                (self.X[3::self.n][:-1] - self.X[4::self.n][:-1]) / self.L))
        # Velocity (v_r, v_l)
        gv_r = self.X[3::self.n][1:] - self.X[3::self.n][:-1] - 0.5 * self.h * (
                self.X[5::self.n][1:] + self.X[5::self.n][:-1])
        gv_l = self.X[4::self.n][1:] - self.X[4::self.n][:-1] - 0.5 * self.h * (
                self.X[6::self.n][1:] + self.X[6::self.n][:-1])
        # Positive linear velocity
        gv = self.X[3::self.n][:-1] + self.X[4::self.n][:-1] - 0.02
        # Minimum angular velocity
        gw_min = (((self.X[3::self.n][:-1] - self.X[4::self.n][:-1]) / self.L) * (
                (self.X[3::self.n][:-1] - self.X[4::self.n][:-1]) / self.L)) - 0.0025
        # Maximum angular velocity
        gw_max = 0.81 - (((self.X[3::self.n][:-1] - self.X[4::self.n][:-1]) / self.L) * (
                (self.X[3::self.n][:-1] - self.X[4::self.n][:-1]) / self.L))

        self.g = ca.vertcat(gx, gy, gtheta, gv_r, gv_l, gv, gw_min, gw_max)

    def main_NMPC(self, step, step_tot, x_ref, y_ref, theta_ref, vr_ref, vl_ref, ar_ref, al_ref, X0, occupancy_message,
                  x_obstacles_all, y_obstacles_all, position_pub):
        # --- Constraints ---
        ...
        if i_coords != []:
            ...
            midpoints = np.column_stack(
                ((i_coords + 0.5) * self.voxel_size + min_x, (j_coords + 0.5) * self.voxel_size + min_y))
            ...
            for i in range(self.N):
                for voxel_midpoint in voxel_midpoints:
                    obstacle_constraints[k] = (self.X[0::self.n][i] - voxel_midpoint[0]) * (
                            self.X[0::self.n][i] - voxel_midpoint[0]) + (
                                                      self.X[1::self.n][i] - voxel_midpoint[1]) * (
                                                      self.X[1::self.n][i] - voxel_midpoint[1]) - (
                                                      self.d_safe * self.d_safe)
                    k += 1
        ...
        if step == 0:
            self.g = ca.vertcat(self.g, self.X[0::self.n][0] - X0[0], self.X[1::self.n][0] - X0[1],
                                self.X[2::self.n][0] - X0[2], self.X[3::self.n][0] - X0[3],
                                self.X[4::self.n][0] - X0[4],
                                obstacle_constraints)  # self.X[0::self.n][-1] - x_ref[step+(self.N-1)], self.X[1::self.n][-1] - y_ref[step+(self.N-1)])
        else:
            self.g = self.g[:(multiple_constraints * (self.N - 1))]
            self.g = ca.vertcat(self.g, self.X[0::self.n][0] - X0[0], self.X[1::self.n][0] - X0[1],
                                self.X[2::self.n][0] - X0[2], self.X[3::self.n][0] - X0[3],
                                self.X[4::self.n][0] - X0[4], obstacle_constraints)

        # --- Cost function ---
        J = 0
        for i in range(self.N):
            # Reference following error cost
            ref_follow_error = ((self.X[0::self.n][i] - x_ref[i + step]) * (self.X[0::self.n][i] - x_ref[i + step])) + (
                    (self.X[1::self.n][i] - y_ref[i + step]) * (self.X[1::self.n][i] - y_ref[i + step]))

            # Successive control cost
            if i != (self.N - 1):
                successive_error = ((self.X[5::self.n][i + 1] - self.X[5::self.n][i]) * (
        ...
        if step == 0:
            init_guess = [
                [x_ref[:self.N][i], y_ref[:self.N][i], theta_ref[:self.N][i], vr_ref[:self.N][i], vl_ref[:self.N][i],
                 ar_ref[:self.N][i], al_ref[:self.N][i]] for i in range(len(x_ref[:self.N]))]
            init_guess = ca.vertcat(*init_guess)
        else:
            init_guess = ca.vertcat(self.opt_states[7:], x_ref[step + self.N - 1], y_ref[step + self.N - 1],
                                    theta_ref[step + self.N - 1], vr_ref[step + self.N - 1], vl_ref[step + self.N - 1],
                                    ar_ref[step + self.N - 1], al_ref[step + self.N - 1])
...
def controller_node(x_traj, y_traj, theta_traj, t_traj, vr_traj, vl_traj, ar_traj, al_traj):
    ...
    f, h, r, L, v_max, a_max, step_tot, N, lambda_1, step, d_safe, voxel_size, max_range, d_replan = variable_definition(
        t_traj)
    ...
    while step <= step_tot and not rospy.is_shutdown():
        ...
        if step >= step_tot - (N - 1):
            ...
            try:
                ...
                rospy.sleep(0.1)
            X0 = [pose.transform.translation.x, pose.transform.translation.y,
                  quaternion_to_yaw(pose.transform.rotation), vr_opt[N - (step_tot - step) - 1],
                  vl_opt[N - (step_tot - step) - 1]]
        ...
        # Normal closed-loop commands
            ...
            if step == 0:
                ...
                X0 = [pose.transform.translation.x, pose.transform.translation.y,
                      quaternion_to_yaw(pose.transform.rotation), 0, 0]
            else:
                X0 = [pose.transform.translation.x, pose.transform.translation.y,
                      quaternion_to_yaw(pose.transform.rotation), vr_opt, vl_opt]

            X0_all.append(X0[:3])

            # --- Solve the optimal control problem and get optimum controls for next sampling period ---
            # Solve the optimal control problem
            vr_opt, vl_opt, x_obstacles_all, y_obstacles_all, solver_time, data_time = nmpc.main_NMPC(step, step_tot,
                                                                                                      x_ref, y_ref,
                                                                                                      theta_ref, vr_ref,
                                                                                                      vl_ref, ar_ref,
                                                                                                      al_ref, X0,
        ...
        if step <= step_tot - (N - 1):
            dist = np.sqrt(
                ((X0[0] - x_ref[step]) * (X0[0] - x_ref[step])) + ((X0[1] - y_ref[step]) * (X0[1] - y_ref[step])))
            if dist > d_replan:
