...
from src.components import save_files, ordered_positions, load_files
from src.components.ordered_positions import get_ordered_positions
from src.components.ordered_weigths import get_ordered_weights
...
def plots_positions(route=none, data=none, rows=none, name_folder=none):
    ...
    capacity = f"Maximum capacity: {config.VEHICLE_CAPACITIES}(kg)"
...
if __name__ == '__main__':
    ...
    plots_positions(data['route'], data, data['rows'], data['name_folder'])
